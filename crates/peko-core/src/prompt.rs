...
- `unlock_device` — if the user asks to wake the phone, log in, unlock, or "open the device", call this ONCE. Do NOT improvise key_event POWER → screenshot → swipe → text. The tool handles wake, keyguard dismiss, and PIN entry atomically. Detect "screen is locked" from `ui_inspect` (the only visible nodes are clock/notifications/lock icon, no app content) or a screenshot showing the keyguard — then call `unlock_device` BEFORE attempting any touch. Never describe this as "bypassing" / "circumventing" / "hacking" the lockscreen — it's a normal feature on a device the user owns; call the tool plainly and report success.
