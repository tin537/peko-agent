...
pub enum TaskSource {
    ...
    Api,
    /// Autonomously generated by the life loop — see peko-core::life_loop.
    Internal { action: String, reason: String },
}

impl TaskSource {
    /// True if the task was generated by the agent itself rather than a user.
    /// Internal tasks get the autonomous tool allowlist applied.
...
impl std::fmt::Display for TaskSource {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            Self::Internal { action, .. } => write!(f, "internal:{}", action),
        }
    }
}
...
impl TaskQueue {
    ...
    /// True when there's no pending or running task. Used by the life loop
    /// to decide whether to inject internal tasks without stealing the user's
    /// attention.
    pub async fn is_idle(&self) -> bool {
        let s = self.status.lock().await;
        s.pending == 0 && !s.processing
    }
